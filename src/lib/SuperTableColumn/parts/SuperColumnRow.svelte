<script>
  ...
  $: if (columnOptions.hasChildren && contents)
  ...
  const getCellValue = (value) => {
    return columnOptions.template
      ? processStringSync(columnOptions.template, { value })
      : undefined;
  };
</script>
...
>
  <Provider
    data={{ id: row?.rowID, value: row?.rowValue, row: {} }}
  ...
  >
    ...
    {:else}
      <div
        ...
        style:justify-content={columnOptions.align}
      >
        <slot />
      </div>
    {/if}
